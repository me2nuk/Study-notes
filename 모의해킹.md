...
__1. Initial Compromise__

Initial Compromise는 첫번째 침투 단계로 침투를 하기 위해 사용되는 다양환 기술들이 있다.
예로 Spear Phishing, Account Hijacking, Watering Hole 등이 존재합니다.

__2. Establish Foothold__

Establish Foothold는 악성코드를 특정 사용자 컴퓨터에 설치를 했다면 거점확보를 하는 단계라고 생각하면 된다.

__3. Privilege Escalation__

Privilege Escalation는 권한 상승에 꼭 필요한 단계이다.

__4. Internal Reconn__

Internal Reconn는 어떠한 바이러스로 권한을 흭득을 하였다면 내부 네트워크 정찰 단계라고 볼 수 있다.

__5. Lateral Movement__

Lateral Movement는 local 네트워크에 존재하는 다른 시스템의 제어 권한을 희득하기 위해 존재하는 단게이다.

__6. Maintain Persistent__

Maintain Persistent는 어떠한 바이러스로 인데 권한을 얻는데에 성공했다면 접근에 대한 유지를 확보한다.

__7. Mission Complete__

Mission Complete는 간단하게 정찰 파일 정보 일출 단계이다.

[자료](https://www.youtube.com/watch?v=lg_VAt6KzmM&feature=emb_logo)
...
XSS 와 CSRF 에서 둘이 비슷한 공통점은 어떠한 스크립트를 삽입하여 데이터를 노출하게 만드는것이다.

하지만 엄연히 각 각의 기법들로 구별이 되는 취약점이다.

해당 XSS 와 CSRF 취약점 기법의 차이점을 정의하자면

XSS 는 클라이언트(유저)가 공격 대상인 반면에 CSRF 는 서버(관리자)가 공격 대상이 되어버린다.

즉 둘이 악성 스크립트를 삽입하는 구조는 비슷하지만 공격 대상이 달라지면서 서로 확연히 차이나는 취약점으로 나오게 된다.
